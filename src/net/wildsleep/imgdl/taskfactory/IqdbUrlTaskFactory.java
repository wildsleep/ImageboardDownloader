...
public class IqdbUrlTaskFactory implements UrlTaskFactory {
	...
	public IqdbUrlTaskFactory(ValueStrategy<File> directoryStrategy, int minimumSimilarity, IqdbPostPrioritization postPrioritization) {
	...
	public Task create(final URL url) {
		ValueStrategy<URL> sourceStrategy = new ValueStrategy<URL>() {
			@Override
			public URL get() {
				return url;
			}};
		final IqdbQueryTask queryTask = IqdbQueryTask.makeUrlQueryTask(sourceStrategy);
